// Bethae -- lightweight ops view backend (2026-08-02).
//
...
module.exports = async (req, res) => {
  ...
  try {
    const families = await fetchFromSupabase(
      ...
      '/rest/v1/families?select=id,email,status,created_at'
    );
    ...
    for (const f of families) {
      ...
      if (created && created >= weekAgo) {
        signupsThisWeek.push({ email: f.email, created_at: f.created_at, status: f.status });
      }
    }
